...
router.use((req, res, next) => {
  console.log("A request is coming to course route");
  next();
});
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res.status(400).send("New course cannot been saved.");
  }
});
...
router.patch("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      msg: "Only the instructor or admin can edit this course.",
    });
  }
});
...
router.delete("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      msg: "Only the instructor or admin can delete this course.",
    });
  }
});
